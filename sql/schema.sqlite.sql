...
CREATE TABLE IF NOT EXISTS sites (
  ...
  name                     TEXT    NOT NULL,
  url                      TEXT    NOT NULL,
  monitor_type             TEXT    NOT NULL DEFAULT 'active' CHECK (monitor_type IN ('active','heartbeat')),
  method                   TEXT    NOT NULL DEFAULT 'GET',
  interval_seconds         INTEGER NOT NULL DEFAULT 60,
  timeout_ms               INTEGER NOT NULL DEFAULT 10000,
  check_type               TEXT    NULL DEFAULT 'status' CHECK (check_type IS NULL OR check_type IN ('status','string','json')),
...
CREATE TABLE IF NOT EXISTS channels (
  id          INTEGER PRIMARY KEY AUTOINCREMENT,
  name        TEXT    NOT NULL,
  type        TEXT    NOT NULL CHECK (type IN ('discord','webhook','email')),
